...
**12. The channel wire contract is bootstrap-level, not per-tool.** Every generated agent assumes a fronting channel middleware: identity arrives in the invoke input as snake_case `user_id` / `customer_code` / `role` (absent ⇒ `undefined` — coerce with `?? null` before writing into validated structures), and channel handoffs are signalled via the `pendingHandoff` slot + the post-model hook in `agent.ts` that stamps the final reply with the `is_handoff` `additional_kwargs` contract. These ship in the bootstrap templates — never rename the fields, never strip the hook. Handoff actions are `soleStep` and follow `templates/executor-handoff.ts.template`. Agents are role-typed at bootstrap: an **orchestrator** hands off to the specialized agents (in-domain topics are handled or routed, never refused; out-of-domain gets a one-line steer-back, not a goodbye); a **specialized** agent hands back ONLY to the orchestrator with `handoff_type` ∈ {`COMPLETED`, `ABANDON`, `OFF_TOPIC`} — same slot, same hook, different catalog and prompt policy. See `streaming-and-channel-contract.md`.
